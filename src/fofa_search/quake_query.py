        ...
        for item in data["data"]:  # 遍历每个数据项
            ...
            record = {
                ...
                "page_type": http_service.get('page_type', [""])[0].replace('\n',
                                                                            '') if 'page_type' in http_service and
                                                                                   http_service.get(
                    'page_type') else "",  # 页面类型
...
def quake_search_page(QUAKE_TOKEN, DB_USER, DB_PASSWORD, DB_NAME, DB_HOST):
    results = []
    error_msg = None
    size = 20
            ...
            if results:
                try:
                    with create_db_connection(DB_USER, DB_PASSWORD, DB_NAME, DB_HOST) as conn:
                        ...
                        save_search_history(conn, 'search_history', query, results, 'quake')
